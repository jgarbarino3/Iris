...
| P2-04 | active   | Implement file-atomic batches, explicit subset batches, overlap/order rules, and best-effort multi-file compensation | Batch service / injected-failure tests            | P2-03        |
...
- P2-03 verified: selection and file/range review acceptance now preserve proposal-time project/file/range identity, preflight strict hash/text/anchor invariants, use the same acknowledged batch adapter, persist receipt-backed terminal state, and leave no production direct replacement acceptance writer. P2-04 is active but not implemented.
