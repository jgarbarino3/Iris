...
| P2-05 | active   | Implement strict unique-anchor rebase, conflict preview, retarget, and supersede                                     | Resolver / repeated-text browser tests            | P2-03        |
...
- P2-04 verified: explicit transaction-ID subsets are durable and idempotent; each file preflights as one exact atomic batch and mutates through one acknowledged CodeMirror dispatch; deterministic ordering and overlap rejection are enforced; a first-file failure atomically fails/skips later batches while returning their untouched transactions to `proposed`; later-file failure stops untouched files and compensates earlier files in reverse order; failed compensation enters terminal recovery-required state with a redacted exportable bundle. P2-05 is active but not implemented.
