...
- The current implementation passes 387 root tests, root formatting/typecheck/build, 318 host tests, host formatting/typecheck/build, and the Playwright smoke.
...
Execute P1-06 only: separate runtime permission policy from `documentEditMode=review`, migrate settings labels and stored options safely, and keep the Phase 2 transaction cutover out of scope.
